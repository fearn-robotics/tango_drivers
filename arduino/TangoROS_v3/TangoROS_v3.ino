//https://wiki.dfrobot.com/12V_DC_Motor_146rpm_w_Encoder_SKU_FIT0277_#Encoder_Sample_Code
#include <ros.h>
#include <ros/time.h>
...
void setup(){
  ...
  for (byte i=0; i<nLowerRelays; i++){
    ...
    if (arcRobot == false){
      ...
      relayOnOff(i, 1); //turn everything off on startup
    }
  }
  for (byte i=0; i<nUpperRelays; i++){
    ...
    if (arcRobot == false){
      ...
      relayOnOff(i+nUpperRelays, 1); //turn everything off on startup
    }
  }
  if (arcRobot == false){
